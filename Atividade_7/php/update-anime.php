...
// Carrega apenas se o anime pertencer ao usuário logado
$id = $_GET['id'];
...
if (!$anime) {
    $_SESSION['error'] = "Você não tem permissão para editar este anime.";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $pdo->prepare("UPDATE animes SET nome = ?, genero = ?, data_publicacao = ?, nota = ? WHERE id = ? AND user_id = ?");

    $stmt->execute([$nome, $genero, $data_publicacao, $nota, $id, $user_id]);
